...
이 포크는 업스트림과 구분되면서 **어느 업스트림 버전을 기반으로 했는지**가 드러나는
태그 규칙을 씁니다.
...
| 태그 | 최종 버전 | 의미 |
|---|---|---|
| `v1.0.235M1` | `1.0.235M1` | 업스트림 1.0.235 기반, 내부 1차 |
| `v1.0.235M2` | `1.0.235M2` | 같은 기반, 내부 2차 수정 |
| `v1.0.240M1` | `1.0.240M1` | 업스트림 1.0.240으로 리베이스한 뒤 1차 |

업스트림은 `v1.0.235` 형태만 쓰므로 태그도 버전 문자열도 겹치지 않습니다.
...
`M` 앞에 **하이픈을 넣지 않는 이유가 여기 있습니다.** 위 로직은 하이픈 뒤를 커밋 수로
해석하므로, `v1.0.235-M1`은 `1.0.236-nightly.0`이 되어 표식이 사라지고 패치 버전까지
올라갑니다. 하이픈 없는 `v1.0.235M1`은 변환을 타지 않고 그대로 통과합니다.

| `git describe` 결과 | 최종 버전 |
|---|---|
| `v1.0.235M1` | `1.0.235M1` ✓ |
| `v1.0.235M1-3-gabc1234` | `1.0.236M1-nightly.0` (태그 이후 3커밋) |
| `v1.0.235-M1` | `1.0.236-nightly.0` ✗ 하이픈 금지 |

> 태그 이후 커밋에서 표식을 유지하려면 [scripts/vars.mjs](scripts/vars.mjs)에 한 군데 보강이
> 필요했습니다. 원래 로직은 `semver.inc('1.0.235M1-c3-g...')`가 **null**을 돌려주는 것을
> 처리하지 않아 `Cannot read properties of null`로 죽었고, 이 파일은 루트 `postinstall`의
> `install-deps.mjs` / `build-native.mjs`가 import하므로 **`yarn install` 자체가 실패**했습니다.
> 표식 없는 업스트림 태그(`v1.0.235`, `v1.0.235-3-g...`)의 결과값은 바뀌지 않습니다.

`1.0.235M1`은 엄격한 semver는 아니지만 electron-builder가 `loose` 모드로 검증하므로
(`semver.valid('1.0.235M1', true)` → `1.0.235-M1`) 문제없이 통과하고, Windows 버전 형식도
`parseInt`가 `235M1`을 `235`로 읽어 `1.0.235.<run_number>`가 됩니다.
자동 업데이트를 제거했으므로 semver 정밀도에 의존하는 로직은 없습니다.

### 반드시 알아둘 점

**태그를 새로 달지 않고 커밋만 올리면 버전이 `1.0.236-nightly.0`이 됩니다.**
`git describe`가 `v...-<커밋수>-g<sha>`를 반환하기 때문입니다. 정식 산출물을 내려면
릴리즈 커밋에 태그를 달아야 합니다.

**이미 태그가 있는 커밋에 새 태그를 겹쳐 달지 마세요.** 한 커밋에 `v1.0.235`와
`v1.0.235M1`이 함께 있으면 `git describe`가 어느 쪽을 고를지 보장되지 않습니다
(실측 시 기존 `v1.0.235`가 선택됨). 변경사항을 커밋한 **새 커밋**에 태그하세요.

**Windows FileVersion 구분**: `M1`/`M2`는 `parseInt`에서 잘려 나가므로, 워크플로가
`BUILD_NUMBER: ${{ github.run_number }}`를 넘겨 4번째 자리를 채웁니다
(`1.0.235.<run_number>`). 로컬 빌드에서 구분이 필요하면 `BUILD_NUMBER`를 직접 지정하세요.
...
# 2) 릴리즈 태그: v<업스트림 버전>M<내부 차수>  (M 앞에 하이픈 금지)
git tag v1.0.235M1

# 3) 브랜치와 태그를 각각 push
git push origin master
git push origin v1.0.235M1
```
